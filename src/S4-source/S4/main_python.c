...
static PyObject *S4SpectrumSampler_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	...
	double x0, x1;
	SpectrumSampler_Options options = {33, 0.001, 0, 1e-6, 0};
	...
	if(!isfinite(x0) || !isfinite(x1) || x0 == x1 ||
		options.initial_num_points < 3 || !isfinite(options.range_threshold) ||
		options.range_threshold < 0 || !isfinite(options.min_dx) || options.min_dx <= 0 ||
		!isfinite(options.max_bend) || options.max_bend < 0 || options.max_bend > 1){
		PyErr_SetString(PyExc_ValueError, "SpectrumSampler requires distinct finite endpoints, "
			"at least 3 initial points, nonnegative RangeThreshold, positive MinimumSpacing, "
			"and MaxBend in [0,1] (cosine of the bend angle).");
		return NULL;
	}
...
static PyObject *S4Sim_SetMaterial(S4Sim *self, PyObject *args, PyObject *kwds){
	...
	id = S4_Simulation_GetMaterialByName(self->S, name);
	if(0 == epsdata.type){
		id = S4_Simulation_SetMaterial(self->S, id, name, S4_MATERIAL_TYPE_SCALAR_COMPLEX, epsdata.eps);
	}else{
		double eps[10] = {epsdata.eps[0], epsdata.eps[1], epsdata.eps[2], epsdata.eps[3],
			epsdata.eps[6], epsdata.eps[7], epsdata.eps[8], epsdata.eps[9], epsdata.eps[16], epsdata.eps[17]};
		id = S4_Simulation_SetMaterial(self->S, id, name, S4_MATERIAL_TYPE_XYTENSOR_COMPLEX, eps);
	}
	if(id < 0){
		PyErr_SetString(PyExc_RuntimeError, "SetMaterial failed");
		return NULL;
	}
...
static PyObject *S4Sim_AddLayer(S4Sim *self, PyObject *args, PyObject *kwds){
	...
	S4_MaterialID material = S4_Simulation_GetMaterialByName(self->S, matname);
	if(material < 0 || thickness < 0){
		PyErr_SetString(PyExc_ValueError, "AddLayer requires an existing material and nonnegative thickness");
		return NULL;
	}
	if(S4_Simulation_SetLayer(self->S, -1, name, &thickness, -1, material) < 0){
		PyErr_SetString(PyExc_RuntimeError, "AddLayer failed"); return NULL;
	}
...
static PyObject *S4Sim_SetLayer(S4Sim *self, PyObject *args, PyObject *kwds)
{
	...
	if(NULL == layer){
		if(NULL == material){ PyErr_SetString(PyExc_ValueError, "A new layer requires a material"); return NULL; }
		return S4Sim_AddLayer(self, args, kwds);
	}
	else
	{
		if(thickness < 0){ PyErr_SetString(PyExc_ValueError, "Thickness must be nonnegative"); return NULL; }
		if(NULL != material){
			S4_MaterialID mid = S4_Simulation_GetMaterialByName(self->S, material);
			if(mid < 0){ PyErr_SetString(PyExc_ValueError, "Unknown material"); return NULL; }
			S4_LayerID lid = S4_Simulation_GetLayerByName(self->S, name);
			if(S4_Simulation_SetLayer(self->S, lid, NULL, &thickness, -1, mid) < 0){
				PyErr_SetString(PyExc_RuntimeError, "SetLayer failed"); return NULL;
			}
		}else{
...
static PyObject *S4Sim_AddLayerCopy(S4Sim *self, PyObject *args, PyObject *kwds){
	...
	S4_LayerID copy = S4_Simulation_GetLayerByName(self->S, layername);
	if(copy < 0 || thickness < 0){
		PyErr_SetString(PyExc_ValueError, "AddLayerCopy requires an existing layer and nonnegative thickness");
		return NULL;
	}
	if(S4_Simulation_SetLayer(self->S, -1, name, &thickness, copy, -1) < 0){
		PyErr_SetString(PyExc_RuntimeError, "AddLayerCopy failed"); return NULL;
	}
...
static PyObject *S4Sim_SetFrequency(S4Sim *self, PyObject *args){
	...
	double frequency[2] = {f.real, f.imag};
	S4_Simulation_SetFrequency(self->S, frequency);
	if(self->S->omega[0] <= 0){
...
static PyObject *S4Sim_GetPowerFlux(S4Sim *self, PyObject *args, PyObject *kwds){
	...
	ret = S4_Simulation_GetPowerFlux(self->S, (S4_LayerID)(layer - self->S->layer), &offset, power);
	if(0 != ret){
...
static PyObject *S4_SolveInParallel(PyObject *Self, PyObject *args, PyObject *kwds)
{
	PyErr_SetString(PyExc_NotImplementedError,
		"The legacy SolveInParallel function was never implemented. "
		"Use s4_parallel.parallel_map with a top-level simulation function.");
	return NULL;
}

static PyObject *S4_AccelerationInfo(PyObject *self, PyObject *args) {
#ifdef HAVE_S4_ACCEL
	return Py_BuildValue("{s:s,s:K,s:K,s:K}", "status", S4_accel_status(),
		"cpu_gemm_calls", S4_accel_cpu_calls(), "gpu_gemm_calls", S4_accel_gpu_calls(),
		"gpu_failures", S4_accel_gpu_failures());
#else
	return Py_BuildValue("{s:s}", "status", "Reference build (internal matrix routines)");
#endif
}

static PyMethodDef S4_funcs[] = {
	{"AccelerationInfo", S4_AccelerationInfo, METH_NOARGS, PyDoc_STR("Return backend status and per-process GEMM counters.")},
