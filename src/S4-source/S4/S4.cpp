...
S4_Simulation* S4_Simulation_New(const S4_real *Lr, unsigned int nG, int *G){
	...
	S4_Simulation *S = (S4_Simulation*)calloc(1, sizeof(S4_Simulation));
...
S4_Simulation* S4_Simulation_Clone(const S4_Simulation *S){
	...
	// Construct independent owned storage; shallow-copying the old structure
	// shared G/k arrays and solution caches and corrupted the source on destroy.
	S4_Simulation *T = S4_Simulation_New(S->Lr, S->n_G, S->G);
	if(NULL == T){ return NULL; }
	T->options = S->options;
	if(S->options.vector_field_dump_filename_prefix){
		T->options.vector_field_dump_filename_prefix = strdup(S->options.vector_field_dump_filename_prefix);
	}
	memcpy(T->omega, S->omega, sizeof(T->omega));
	memcpy(T->k, S->k, sizeof(T->k));
	T->msg = S->msg;
	T->msgdata = S->msgdata;
	for(int i = 0; i < S->n_materials; ++i){
		const S4_Material *M = &(S->material[i]);
		const int type = M->type == 0 ? S4_MATERIAL_TYPE_SCALAR_COMPLEX : S4_MATERIAL_TYPE_XYTENSOR_COMPLEX;
	...
	for(int i = 0; i < S->n_layers; ++i){
		...
		for(int j = 0; j < L->pattern.nshapes; ++j){
			if(L->pattern.shapes[j].type == POLYGON){
				const size_t bytes = sizeof(double)*2*L->pattern.shapes[j].vtab.polygon.n_vertices;
				L2->pattern.shapes[j].vtab.polygon.vertex = (double*)S4_malloc(bytes);
				memcpy(L2->pattern.shapes[j].vtab.polygon.vertex, L->pattern.shapes[j].vtab.polygon.vertex, bytes);
			}
		}
...
S4_message_handler S4_Simulation_SetMessageHandler(
	...
	if(NULL == S){ return NULL; }
	S4_message_handler previous = S->msg;
	S->msg = handler;
	S->msgdata = data;
	return previous;
}
...
S4_MaterialID S4_Simulation_SetMaterial(
	...
	S4_Material *M = NULL;
	// A changed dielectric invalidates cached eigenmodes and solved fields.
	S4_Simulation_DestroyLayerModes(S, -1);
	Simulation_DestroySolution(S);
	Simulation_InvalidateFieldCache(S);
	if(id < 0){
